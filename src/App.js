...
import AnimatedLogo from "./AnimatedLogo"; // AnimatedLogoコンポーネントをインポート
...
const App = () => {
  ...
  useEffect(() => {
    ...
    const loadTasks = async () => {
      // 実際のアプリではここでAPIやローカルストレージからデータを取得します
    ...
    switch (task.recurrence.type) {
      ...
      case "weekly":
        return (
          ...
          ((today - task.startDate) / (7 * 24 * 60 * 60 * 1000)) %
      ...
      case "custom":
        const diffDays = Math.floor(
          (today - task.startDate) / (24 * 60 * 60 * 1000)
        );
...
const TaskForm = ({ addOrUpdateTask, categories, onClose, editingTask }) => {
  ...
  const [intervalUnit, setIntervalUnit] = useState(
    editingTask ? editingTask.recurrence.intervalUnit : "day"
  );
  ...
  const [occurrences, setOccurrences] = useState(
    editingTask ? editingTask.endCondition.occurrences : 1
  );
